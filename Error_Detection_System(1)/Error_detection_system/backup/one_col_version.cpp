...
int main(void) {
    ...
    for(i = 0;i < row; i++){
        learnMap[i] = (char *)malloc(sizeof(char) * MAX_STR);
    }
    ...
    for(i = 0; i < t_row; i++){
        for(j = i+1; j < t_row; j++){
            /*for(h = 0; h < t_col; h++){*/
            if(strcmp(table[i][0],table[j][0]) == 0){
                l=0;
                while(l < t_row){
                    if(strcmp(learnMap[l],table[i][0]) == 0){
                        flag =0;
                        break;
                    }
                    flag += abs(strcmp(learnMap[l],table[i][0]));
                    l++;
                    
                }
                if(flag > 0){
                    strcpy(learnMap[k],table[i][0]);
                    k++;
                    flag = 0;
                    break;
                }
    ...
    for(j = 0; j < k;j++){
        for(i = 1;i < t_row;i++){
            result = getJaroWinklerDistance(table[i][0], learnMap[j], 0.1);
            printf("\"%s\"|\"%s\" 類似度：%f\n",table[i][0],learnMap[j],result);
        }
    }
