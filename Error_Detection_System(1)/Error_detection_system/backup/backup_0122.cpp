...
//
//  Created by 松尾静一郎 on 2017/12/22.
//  Copyright © 2017年 松尾静一郎. All rights reserved.
//
...
int main(void) {
    ...
    for(j = 0; j < k;j++){
        for(i = 1;i < t_row;i++){
            result[i] = getJaroWinklerDistance(table[i][0], learnMap[j][0], 0.1);
            printf("\"%s\"|\"%s\" 類似度：%f\n",table[i][0],learnMap[j][0],result[i]);
        }
    }
